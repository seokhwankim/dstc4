...
def main(argv):
	...
	checker = TrackChecker(sessions, system_output, tagsets, args.pilottask, args.roletype)
	checker.check()
	checker.print_errors()

class TrackChecker():
	def __init__(self, sessions, tracker_output, tagsets, tasktype, roletype):
	...
	# first check the top-level stuff
		...
		elif self.tracker_output['task_type'] != self.tasktype:
		...
		for session, track_session in zip(self.sessions, self.tracker_output["sessions"]):
			...
			for log_utter, label_utter, track_utter in zip(log_utter_list, label_utter_list, track_session["utterances"]):
				...
				if self.tasktype == 'SLU' or self.tasktype == 'SAP':
				...
				if self.tasktype == 'SLG' or self.tasktype == 'EES':
					if 'generated_sentence' not in track_utter:
						self.add_error((session_id, "utterance", log_utter['utter_index']), "no generated_sentence key in utterance")
					else:
						if type(track_utter['generated_sentence']) != types.StringType and type(track_utter['generated_sentence']) != types.UnicodeType:
							self.add_error((session_id, "utterance", log_utter['utter_index'], type(track_utter['generated_sentence'])), "a value for 'generated_sentence' key should be a string")
