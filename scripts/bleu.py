...
# For DSTC4 we have introduced some changes to allow calculating the score for each sentence. All changes start with the
# word DSTC4
...
score_set(s, testid, refids, n=4): Interface with datasetBleu.py; calculate BLEU score of testid against refids.
...
import datasetBleu  # DSTC4: Changed name
#import log
...
def cook_test(test, (reflens, refmaxcounts), n=4):
    ...
    # DSTC4: To skip empty lines
    if len(test) == 0:
        result["testlen"] = 1  # DSTC4: This trick avoids the system to crash
...
def score_cooked(allcomps, n=4):
    ...
    for k in xrange(n):
        if totalcomps['correct'][k] == 0:
            return 0.0
        # log.write("%d-grams: %f\n" % (k,float(totalcomps['correct'][k])/totalcomps['guess'][k]))  # DSTC4: Commented out to have a cleaner output
        logbleu += math.log(totalcomps['correct'][k])-math.log(totalcomps['guess'][k])
    logbleu /= float(n)
    # log.write("Effective reference length: %d test length: %d\n" % (totalcomps['reflen'], totalcomps['testlen']))  # DSTC4: Commented out to have a cleaner output
    logbleu += min(0,1-float(totalcomps['reflen'])/totalcomps['testlen'])
    return math.exp(logbleu)


# DSTC4: Introduced to keep track of the score for each sentence. It is based on score_cooked function
    ...
    for comps in allcomps:
        ...
        for k in xrange(n):
            if totalcomps['correct'][k] == 0:
                ...
                continue
            num_ngram += 1
            bleu_per_ngram.append(float(totalcomps['correct'][k])/totalcomps['guess'][k])
            logbleu += math.log(totalcomps['correct'][k])-math.log(totalcomps['guess'][k])
...
# DSTC4: Allows the module to be call from another script. By default we read raw_test.
# This function is based on the process done in the main function
def calculateBLEU(ref, tst, pc=False, ngram_order=4):
    global preserve_case
    s = datasetBleu.Dataset()
    if pc is True:
        preserve_case = pc
...
if __name__ == "__main__":
    # import psyco  # DSTC4: Commented as they are just to optimize the calculation process
    # psyco.full()
