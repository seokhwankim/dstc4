...
# $Id: dataset.py 1307 2007-03-14 22:22:36Z hieuhoang1972 $
'''Decoder interface:
...
import sys, sgmllib, xml.sax.saxutils
# import log
import codecs
...
class Dataset(object):
    ...
    # DSTC4: Allows the addition of a single sentence instead of reading it from a file
    def add_sent(self, line, docid, setid=None, sysid=None, lang=None):
        """Add a sentence"""
        if setid is not None:
            if self.id is not None and self.id != setid:
                raise ValueError, "Set ID does not match"
