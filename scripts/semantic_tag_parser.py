...
class SemanticTagParser(HTMLParser):
	...
	def handle_data(self, data):
		for token in data.strip().split():
			...
			self.__word_tag_seq.append((self.__curr_bio, self.__curr_tag, self.__curr_attrs))
			...
			for c in token:
				...
				self.__chr_tag_seq.append((chr_bio, self.__curr_tag, self.__curr_attrs))
	...
	def tokenize(self, chr_space_seq = None):
		...
		for c, space, tag in zip(self.__chr_seq, self.__chr_space_seq, self.__chr_tag_seq):
			if space:
				if len(curr_word) > 0:
